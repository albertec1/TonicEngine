...
{
	App->renderer3D->DeleteBuffer(mData->mesh_data.index_ID);
	App->renderer3D->DeleteBuffer(mData->mesh_data.vertex_ID);
	App->renderer3D->DeleteBuffer(mData->mesh_data.tex_coords_ID);

	RELEASE_ARRAY(mData->mesh_data.vertex);
	RELEASE_ARRAY(mData->mesh_data.index);
	RELEASE_ARRAY(mData->mesh_data.tex_coords);
	RELEASE_ARRAY(mData->mesh_data.normals);
}
...
void ComponentMesh::Draw()
{
	...
	if (ImGui::CollapsingHeader("Mesh", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		ImGui::SameLine();
		ImGui::TextColored(YELLOW_COLOR, "%d", mData->mesh_data.num_index / 3);

		ImGui::Text("Number of vertices:");
		ImGui::SameLine();
		ImGui::TextColored(YELLOW_COLOR, "%d", mData->mesh_data.num_vertex);

		ImGui::Text("Number of indices:");
		ImGui::SameLine();
		ImGui::TextColored(YELLOW_COLOR, "%d", mData->mesh_data.num_index);

		ImGui::Separator();

		ImGui::Text("File:"); ImGui::SameLine();
		ImGui::TextColored(YELLOW_COLOR, go->GetComponentMesh()->mData->mesh_data.path.c_str());
...
bool ComponentMesh::DrawFaceNormals(GameObject* m, bool active)
{
	if (active)
	{
		...
		for (int j = 0; j < m->GetComponentMesh()->mData->mesh_data.num_index; j += 3)
		{
			// Declaring each vertex of the triangle
			float3 vert1 = m->GetComponentMesh()->mData->mesh_data.vertex[m->GetComponentMesh()->mData->mesh_data.index[j]];
			float3 vert2 = m->GetComponentMesh()->mData->mesh_data.vertex[m->GetComponentMesh()->mData->mesh_data.index[j + 1]];
			float3 vert3 = m->GetComponentMesh()->mData->mesh_data.vertex[m->GetComponentMesh()->mData->mesh_data.index[j + 2]];
...
bool ComponentMesh::DrawVertexNormals(GameObject* m, bool active)
{
	if (active)
	{
		if (m->GetComponentMesh()->mData->mesh_data.normals != nullptr)
		{
			for (int j = 0; j < m->GetComponentMesh()->mData->mesh_data.num_vertex; ++j)
			{
				float3 vert = m->GetComponentMesh()->mData->mesh_data.vertex[j];
				float3 norm = m->GetComponentMesh()->mData->mesh_data.normals[j];
